@section('admin_title')
    <title>پنل مدیریت تخفیف سنسور | تراکنش ها</title>
@endsection
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a class="my-active" href="{{route('transactions.index')}}">مدیریت
                                تراکنش ها</a></li>
                    </ol>
                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{route('transactions.index','status='.\App\Models\Transaction::S_SUCCESS)}}"
                               class="btn btn-success">موفق</a>

                            <a href="{{route('transactions.index','status='.\App\Models\Transaction::S_CANCEL)}}"
                               class="btn btn-warning">در حال تراکنش</a>

                            <a href="{{route('transactions.index','status='.\App\Models\Transaction::S_PENDING)}}"
                               class="btn btn-info">کنسل شده</a>

                            <a href="{{route('transactions.index','status='.\App\Models\Transaction::S_FAIL)}}"
...
<script type="text/javascript">
    ...
    $('#dateForm').on('submit', function (e) {
        e.preventDefault();
        var base_url = '{{cleanExtraQueryString(['token'],null,'transactions.index')}}';
        var route = "{{route('transactions.index')}}";
        var date = $('#date').val();
    ...
    $('#tokenForm').on('submit', function (e) {
        e.preventDefault();
        var base_url = '{{cleanExtraQueryString(['date'],null,'transactions.index')}}';
        var route = "{{route('transactions.index')}}";
        var token = $('#token').val();
