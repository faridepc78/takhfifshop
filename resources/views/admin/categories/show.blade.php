            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{route('categories.index')}}">مدیریت
                                دسته بندی ها</a></li>
                        @if ($category['level']==1)
                            <li class="breadcrumb-item"><a class="my-active" href="{{route('categories.show',$category['id'])}}">مدیریت زیر دسته های ({{$category->name}})</a></li>
                        @elseif($category['level']==2)
                            <li class="breadcrumb-item"><a class="my-active" href="{{route('categories.show',$category['id'])}}">مدیریت زیر دسته های ({{$category->parent->name}}=>{{$category->name}})</a></li>
                        @endif
                    </ol>
                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            @if ($category['level']==1)
                                <h3 class="card-title mb-3">مدیریت زیر دسته های ({{$category->name}})</h3>
                            @elseif($category['level']==2)
                                <h3 class="card-title mb-3">مدیریت زیر دسته های ({{$category->parent->name}}=>{{$category->name}})</h3>
                            @endif
                        </div>
